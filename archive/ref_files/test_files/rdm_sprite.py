...
dot_life = -1               # How many frames a dot follows its trajectory before redrawn. -1
                            # is infinite life
move_distance = 5          #How many pixels the dots move per frame
...
aperture_width = 400;       #How many pixels wide the aperture is. For square aperture this
                            ...
                            #the diameter.
aperture_height = 700;      #How many pixels high the aperture is. Only relevant for ellipse
...
class dot:
    ...
    def reset_location(self):
        ...
        if (aperture_type == 1 or aperture_type == 2):
            ...
            self.x = np.sqrt(rho) * np.cos(phi)
            self.y = np.sqrt(rho) * np.sin(phi)

            #TO-DO: FIND EQUIVALENT WAY TO MAP THESE 2 LINES USING PYGAME
            self.x = self.x * horizontal_axis + aperture_center_x
            self.y = self.y * vertical_axis + aperture_center_y
